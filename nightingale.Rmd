...
Nightingale<- 
  ...
  gather(Date)
colnames(Nightingale)[c(2,3)] <- c("Cause","Deaths")

Nightingale <- Nightingale %>% transform(Date = ymd(as.character(Date)), Cause = sub("\\.rate", "", Cause))

Nightingale <- Nightingale %>% transform(Regime = ifelse(Date < ymd("1855-04-01"), "Before","After"))
